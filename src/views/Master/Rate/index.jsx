...
const Rate = () => {
  ...
  const handleEdit = async (e) => {
    e.preventDefault();
    navigation('/edit-rate-master', {
      state: {
        ...location.state,
        project: 'Edit Master Rate',
        data: {
          rate_id: selectedData[0]?.RATE_ID,
        },
      },
    });
  };
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    navigation('/add-rate-master', {
      state: {
        ...location.state,
        project: 'Add New Rate',
      },
    });
  };
      ...
      <div className='bg-white px-6 pt-10 h-full'>
        <div className='flex sm:flex-row flex-col gap-5 '>
          <div className='sm:w-full'>
            <p className='text-lg font-bold'>Master Rate</p>
            <p className='text-base font-light'>View your list rate in here.</p>
          </div>
        ...
        <div className='flex flex-col gap-5'>
          ...
          <div className={`${(dimensionScreenW < 768 && check) ? 'bringToBack' : ''}`}>
            <TableRate navigation={navigation} location={location} data={dataTable} setData={setDataTable} setSelectedData={setSelectedData} sortBy={sortBy} />
          </div>
        </div>
      </div>
    </>
