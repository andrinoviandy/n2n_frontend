...
const TableCustomer = ({ data, setData, setSelectedData, sortBy }) => {
  const headerTable = ["No", "Kode Akun", "Nama", "NPWP", "Alamat", "Email", "Fax", "Telepon", "Keterangan", "Status"];
            ...
            {tableData?.length > 0 ? tableData?.map((v, i) => {
              return (
                <tr key={i}>
                  ...
                  <td className=''>{(i + 1)}</td>
                  <td className=''>{v.KODE_AKUN || '-'}</td>
                  <td className=''>{v.CUSTOMER_NAME || '-'}</td>
                  <td className=''>{v.NPWP || '-'}</td>
                  <td className=''>{v.ADDRESS || '-'}</td>
                  <td className=''>{v.EMAIL || '-'}</td>
                  <td className=''>{v.FAX || '-'}</td>
                  <td className=''>{v.TELP || '-'}</td>
