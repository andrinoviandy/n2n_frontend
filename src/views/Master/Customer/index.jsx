...
import TableEmployee from './components/TableCustomer';
...
const Customer = () => {
  ...
  const getListTable = async (keyword) => {
    swal.loading();
    try {
      const res = await storeSchema.actions.getListCustomer({
        page: 1,
        limit: 10,
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    navigation('/add-customer-master', {
      state: {
        ...location.state,
      ...
      <div className='bg-white px-6 pt-10 h-full'>
        <div className='flex sm:flex-row flex-col gap-5 '>
          <div className='sm:w-full'>
            <p className='text-lg font-bold'>Master Customer</p>
            <p className='text-base font-light'>View your list customer in here.</p>
          </div>
        ...
        <div className='flex flex-col gap-5'>
          ...
          <div className={`${(dimensionScreenW < 768 && check) ? 'bringToBack' : ''}`}>
            <TableEmployee navigation={navigation} location={location} data={dataTable} setData={setDataTable} setSelectedData={setSelectedData} sortBy={sortBy} />
          </div>
        </div>
      </div>
    </>
