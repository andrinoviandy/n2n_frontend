...
const TableUserManagement = ({ data, setData, setSelectedData, sortBy }) => {
  const headerTable = ["No", "Name", "Role", "Create Date", "Last Login", "Permission", "Status"];
            ...
            {tableData?.length > 0 ? tableData?.map((v, i) => {
              return (
                <tr key={i}>
                  ...
                  <td className='w-[20%]'>{v.PROJECT_NO || '-'}</td>
                  <td className='w-[57%]'>{v.PROJECT_NAME || '-'}</td>
                  <td className='w-[20%]'>{formatCurrency(v.TOTAL_COST) || '-'}</td>
                </tr>
              )
            }) : (
              <tr>
                <td colSpan={headerTable?.length + 1}>
