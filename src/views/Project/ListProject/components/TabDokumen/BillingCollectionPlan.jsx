...
const BillingCollectionPlan = ({ data, getDetailProject, isDetailModalAkselerasi, isBillingRealization }) => {
  ...
  const [headerTable, setHeaderTable] = useState(['Termin', 'Keterangan', 'Divisi', 'Nominal Estimasi Billing', 'Rencana Periode Billing']);
  ...
  const handleUpload = async (e, i) => {
    ...
    try {
      ...
      if (((total + parseInt(value?.est_billing)) > data.NILAI_KONTRAK) && !isBillingRealization) {
        await swal.custom("Tidak Dapat Disimpan !", "Nilai Billing Tidak Dapat Melebihi Nilai Kontrak", "warning");
        getDetailProject();
      } else {
        const payload = {
