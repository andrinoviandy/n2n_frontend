...
const DokumenPenagihan = ({ location, data, getDetailProject }) => {
  const dummyField = {
    dokumen_id: "",
    tipe_dokumen: "01",
    no_dokumen: "",
  ...
  const headerTable = ['No Dokumen', 'Tanggal Penagihan', 'Attachment Dokumen Penagihan']
  ...
  useEffect(() => {
    const dokumenPendukung = data?.DOKUMEN_PENAGIHAN;
  ...
  const handleUpload = async (e, i) => {
    ...
    try {
      ...
      const payload = {
        tipe_dokumen: "01", // dokumen penagihan
        no_dokumen: value?.no_dokumen,
        tgl_dokumen: value?.tgl_dokumen?.substring(0, 10),
        jns_dokumen: "01005",
  ...
  return (
    <>
      <div className='card border-2 my-5'>
        <div className='absolute left-10 -top-3 px-2 bg-white text-sm font-semibold'>Dokumen Penagihan</div>
        <div className='card-body'>
          ...
          <div type="button" disabled={(dataFields.filter(a => a.status.canUpload === true).length) === 1 ? true : false} className='btn btn-sm ml-3 rounded-[25px] border-none bg-blue-50 text-primary mt-3 w-60 ' onClick={handleAddField}>
            <RxPlusCircled size='20px' /> Add Dokumen Penagihan
          </div>
        </div >
      </div >
    </>
...
export default DokumenPenagihan