...
const TableVendorProjectBilling = ({ data, setData, setSelectedData, sortBy, startDate, endDate }) => {
  const headerTable = ["No", "No Project", "Nama Project", "Nama Vendor", "Nomor Kontrak", "Nominal Kontrak", "Termin", "Status Billing"];
  ...
  const renewData = async () => {
    const res = await storeSchema.actions.getListProjectForVendorProjectBilling({
    // const res = await storeSchema.actions.getListBillingByTermin({
      page: currentPage,
      limit: perPage,
...
export default TableVendorProjectBilling