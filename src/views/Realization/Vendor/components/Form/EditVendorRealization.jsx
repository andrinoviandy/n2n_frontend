...
const EditVendorRealization = ({ dataDetail, getDetailProjectVendor }) => {
  ...
  const headerVendorPlanningTable = ["Nama Vendor", "Judul Pengerjaan", "Nominal Estimasi Vendor"];
  const headerListVendor = ["Nama Vendor", "No Kontrak", "Judul Kontrak", "Tanggal Awal Kontrak", "Tanggal Akhir Kontrak", "Nominal Kontrak Vendor"];
  const dummyField = {
    project_vendor_id: '',
    vendor_id: '',
    ur_vendor: '',
  ...
  const handleUpload = async (e, i) => {
    ...
    try {
      ...
      let sumEstimasi = dataDetail?.VENDOR_PLANNING.reduce((total, item) => total + parseInt(item?.NILAI_KONTRAK), 0);
      let sumRealisasi = dataFields?.reduce((total, item) => total + parseInt(item?.nilai_kontrak), 0);

      if (sumRealisasi > sumEstimasi) {
        swal.custom('Tidak Dapat Disimpan !', 'Total Realisasi Tidak Boleh Lebih Besar Dari Total Estimasi', 'warning')
      } else {
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <input
                        ...
                        name='NILAI_KONTRAK'
                        className='input input-sm input-bordered rounded-[25px] w-full bg-white'
                        value={formatCurrency(item?.NILAI_KONTRAK, 0)}
                        disabled
                      />
                    </td>
                  </tr>
              ...
              <tbody>
                {dataFields?.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className='flex min-w-72 gap-3'>
                      <CurrencyInput
                        name='nilai_kontrak'
                        size='-sm'
                        onChange={(value, name) => {
