...
const AddEditVendorRealization = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      let sumRealisasi = dataDetail?.VENDOR_FINAL.reduce((total, item) => {
        return item.PROJECT_VENDOR_ID !== value?.PROJECT_VENDOR_ID ? total + parseInt(item?.NILAI_KONTRAK) : total
      }, 0);

      if ((sumRealisasi + parseInt(value?.NILAI_KONTRAK)) > sumEstimasi) {
        swal.custom('Tidak Dapat Disimpan !', `Total Realisasi (${formatCurrency(sumRealisasi + parseInt(value?.NILAI_KONTRAK), 0)}) Tidak Boleh Lebih Besar Dari Total Estimasi (${formatCurrency(sumEstimasi,0)})`, 'warning')
      } else {
